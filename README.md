...
## 메모리 풀 도입
클라이언트의 요청을 메시지단위로 분할하여 Job객체 형식으로 파싱하는 형태로 구성되어 있음. <br/>
요청마다 새로운 객체를 할당하고 해제하는 것은 비효율적이라고 판단, <br/>
특정 크기의 메모리블록을 ```::operator new```로 할당하고, <br/>
메모리블록에 ```Placement New```를 통해 생성자만 호출하여 사용하는 방식으로 변경. <Br/>
마찬가지로 메모리풀에 반환할 때도 소멸자만 호출한 후 해제하지 않고 메모리풀이 관리한다. <br/>
프로그램 종료 이전에 ```::operator delete```로 해제.
...
이후 설명할 CutTheLine항목에서 선이 끊겨보이는 현상 발생. <br/>
패킷 작성함수에서는 재작성대기시간을 없애고, 송신함수에 0.03초의 텀을 넣는 방식으로 변경. <br/>
...
획 정보 (DrawCommand)를 송신하는 과정도 청크화하였다. <br/>
하나의 청크가 하나의 획을 전달하도록 변경, 획당 가능한 정점수는 2천개 내외 <br/>
(헤더 12바이트, 색상값 3바이트, 선굵기 4바이트. 남은 공간에 정점당 4바이트 할당. 하나의 메시지에 8192바이트를 담을 수 있다.) <br/>

## Texture 처리 개선
초기에는 서버로부터 수신한 ```byte``` 타입 r,g,b 데이터를 각각 ```float```으로 형변환하여 ```Color```객체에 담아 ```SetPixel(x,y,Color)```함수를 호출하는 방법을 사용하였다. <br/>
이 때는 패킷 크기도 2048bytes였기 때문인지 신규유저가 방에 입장하면 **로딩이 3초이상 소요**되었다. <br/>
...
개선한 이후에는 **로딩에 1초 이내로 소요되었다. 로딩시간을 33% 수준으로 개선**하였다. <br/>
...
그러므로 ```Color32[]```로 넘겨주는게 오버헤드가 적게 발생한다. <br/>

획 정보 (DrawCommand)를 청크화하여 수신하는 과정에서도 개선이 있었다. <br/>
타유저가 현재 그리고 있는 획의 정보를 수신할 때에는 바로 캔버스에 반영해주어야하지만, <br/>
방에 신규입장하면서 받는 정보는 그럴 필요 없이 로딩이 끝나면 캔버스에 반영해주어도 괜찮다. <br/>
AddVertexToCommand함수에서 기본적으로는 호출될 때마다 캔버스에 반영하지만, bHasToRenew옵션을 넣어 캔버스에 반영하지 않는 로직도 추가하였다. <br/>
이후 하나의 획 정보 로딩이 완료되면 CommandStack::Push함수에서 GetRenewedTexture함수와 SketchScreen::RenewCanvas함수를 호출하면서 캔버스에 반영한다. <br/>
물론 이 또한 캔버스에 반영하는 부분을 아예 따로 빼도 된다. <br/>

## CutTheLine 요청
하나의 메시지를 특정 크기 이내로 송신하기 위해 큰 데이터를 송신해야하는 CanvasInfo와 DrawCommandInfo에 대해 청크화하여 분할송신하도록 했다. <br/>
하지만, DrawCommand에 정점을 무한히 삽입할 수 있다면, 이 또한 특정 크기를 넘는 메시지를 발생시킬 수 있으므로, <br/>
특정 정점 수를 넘을 때마다 클라이언트가 그리던 선을 끊고 DrawEnd요청 후 다시 이어서 그리는 식으로 획을 분할한다. <br/>

이 과정에서 기작성된 함수인 DrawStart, Draw, DrawEnd요청을 그대로 사용하여 처리하려고 했으나, IOCP 멀티스레드 상에서 문제가 발생. <br/>
Undo동작을 만들기 위해 ReadLock을 사용했었다. 그런데 ReadLock을 거는 함수와 푸는 함수를 서로 다른 네트워크 요청에서 처리해야했기에 발생한 문제. <br/>
그리는 중에 DrawEnd -> DrawStart -> Draw 순으로 요청하여 서버가 순서대로 처리한다면 문제가 없지만, <br/>
메시지가 각각 따로 올 수 있기 때문에 각각의 요청을 다른 스레드가 처리하게 될 수도 있다. <br/>
이 과정에서 ReadLock.Lock()과 ReadLock.Unlock()의 순서가 꼬이는 문제 발생. <br/>

결국 락을 건드리지 않고, 그리던 선을 끊고 다시 새로운 선을 이어나가는 동작이 필요하게 되었다. <br/>
CutTheLine요청으로 명명하고, DrawEnd의 로직 중 ReadLock을 해제하는 동작을 제거하고 작성하였다. <br/>
(DrawStart는 ReadLock을 거는 동작만 수행한다.) <br/>

여기에 추가적인 문제가 발생했는데, 선을 끊고 다음 선을 그리기 시작할 때마다 두 선이 이어지지 않고 사이에 공백이 있는 경우가 종종 발생했다. <br/>
다음 선을 그릴 때, 그 전에 마지막으로 찍었던 정점과, 현재 정점을 Draw요청하게 작성해두어서 문제가 발생할 줄 몰랐다. <br/>
문제가 발생한 원인은, Draw요청을 서버에 전달할 메시지를 작성하는 함수 ReqDraw에서 발생. <br/>
패킷 발생량 자체를 줄이기 위해 Draw요청을 0.03초에 한번만 작성하도록 재작성 대기시간을 만들었었는데, <br/>
이 과정에서 Draw요청 몇개가 서버로 전달되지 않고 누락된 것. <Br/>

이는 재작성 대기시간은 없애고, 송신버퍼의 데이터를 송신하는 함수에서 송신대기시간을 거는 식으로 바꾸어 개선하였다. <br/>
클라이언트가 서버에 전송하는 메시지의 크기는 크지 않다. 그러므로 송신버퍼에 조금 대기해도 괜찮다고 판단하였다.
